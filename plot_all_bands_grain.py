# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 12:45:12 2020
...
display = 'IPFColorX'
grain_list = [154]  # from 2D grain ids
step = 0
...
def plot_all_bands_in_grain(grain_list):
    for grain in grain_list:
        grain2D = int(grain)
        eulers2D = eulers[grainHost2D]
        rotMatrix2D = rot_matrix(eulers2D)
        grain3D = int(feat2Dto3D[grain2D])  # corresponding 3D grain
        create_threshold(grain3D)
        for band in range(grainHosts.shape[1]):
            if grainHosts[step,band] == grain2D:
                origin2D = bandendpoints[step,band,:2]
                origin3D = ([origin2D[0],-12,origin2D[1]])  # -12 comes from the alignment that was done on the extrusion
                ...
                draw_slice(origin_3D,normal_3D,band,grain3D)
plot_all_bands_in_grain(grain_list)