# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 15:26:30 2020
...
display = 'IPFColorX'
grain_of_trd = [154]  # from 2D grain ids
step = 0
...
def plot_all_bands_in_trd(grain_list):
    for grain in grain_list:
        grain2D = int(grain)
        grain3D = int(feat2Dto3D[grain2D])
        trd = int(parentTRD[grain3D])
        for grain_3D in range(parentTRD.shape[0]):
            if parentTRD[grain_3D] == trd:
                create_threshold(grain_3D)
                for grain_2D in range(int(feat2Dto3D.shape[0])):
                    if feat2Dto3D[grain_2D] == grain_3D:
                        for band in range(grainHosts.shape[1]):
                            if grainHosts[step,band] == grain_2D:
                                rotMatrix2D  = rot_matrix(eulers[grain_2D])
                                origin_2D = bandendpoints[step,band,:2]
                                origin_3D = ([origin_2D[0],-12,origin_2D[1]])  # -12 comes from the alignment that was done on the extrusion
                                ...
                                draw_slice(origin_3D,normal_3D,band,grain_3D)

plot_all_bands_in_trd(grain_of_trd)